...
<script lang="ts">
  ...
  function generateApiUrl(): string {
    ...
    isInvalidWeekRange = !isValidStartWeek(startWeek) || !isValidEndWeek(endWeek) || (startWeek > endWeek && endWeek !== '');
...
<section class="stats-widget" class:light={!$theme} class:dark={$theme}>
  <form on:input={handleInput} class:light={!$theme} class:dark={$theme}>
    <label for="stat-category">
      Stat Category:
      <div class="input-container">
        <select id="stat-category" bind:value={selectedCategory} required>
