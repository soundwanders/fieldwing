...
<script lang="ts">
	...
	// Statistics
	$: totalGames = gameResults.reduce(
		(sum, result) => (result.error === null ? sum + result.data.length : sum),
		0
	);
...
<svelte:head>
	...
	<meta
		name="description"
		content="View college football game results and scores for your favorite teams."
	/>
</svelte:head>
...
<ErrorBoundary bind:error={pageError} on:retry={handleRetry}>
	<div class="wrapper">
		{#if isLoading}
			<LoadingSpinner size="large" text="Loading game results..." fullScreen={true} />
		{:else}
									...
									<div class="info-banner">
										<div class="info-content">
											...
											<p class="info-secondary">
												📅 No games scheduled for these teams in Week {week} of {year}
											</p>
											{#if parseInt(year) > new Date().getFullYear()}
												<p class="info-secondary">
													⏭️ Future seasons may not have scheduled games yet
												</p>
											{/if}
										</div>
									</div>
												...
												<div class="game-header">
													<div class="game-meta">
														...
														<span class="season-type"
															>{capitalizeFirstChar(
																gameResult.seasonType || gameResult.season_type
															)}</span
														>
													</div>
													...
													<div class="mobile-matchup">
														...
														<div class="mobile-team away-team">
															<div class="mobile-team-header">
																<h3 class="mobile-team-name">
																	{gameResult.awayTeam || gameResult.away_team || 'Unknown'}
																</h3>
														...
														<div class="mobile-team home-team">
															<div class="mobile-team-header">
																<h3 class="mobile-team-name">
																	{gameResult.homeTeam || gameResult.home_team || 'Unknown'}
																</h3>
												...
												<div class="game-details">
													...
													{#if gameResult.attendance}
														<div class="detail-row">
															<span class="detail-label">👥</span>
															<span class="detail-value"
																>{gameResult.attendance.toLocaleString()} attendance</span
															>
														</div>
													{/if}
								...
								{:else}
									<div class="no-games-card">
										<div class="no-games-content">
											...
											<small
												>This team may have had a bye week or the game data is unavailable.</small
											>
										</div>
									</div>
								{/if}
...
<style>
	...
	/* Mobile Layout (Default - Shows on Mobile) */
	.mobile-matchup {
		display: flex;
		flex-direction: column;
		gap: 0.75rem;
