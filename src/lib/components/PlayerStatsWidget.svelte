<script lang="ts">
	...
	function generateApiUrl(): string {
		...
		const paginationParams = [`limit=${pageSize}`, `skip=${currentPage * pageSize}`].join('&');
	...
	const handleCategoryChange: ChangeHandler<HTMLSelectElement> = (event: {
	...
	<div class="button-container">
		{#if apiUrl && isFormValid}
			<a href={apiUrl} data-sveltekit-reload role="button">
				<button class="submit-button" type="button"> Search Player Stats </button>
			</a>
		{:else}
			<button class="submit-button" type="button" disabled on:click={handleSubmit}>
