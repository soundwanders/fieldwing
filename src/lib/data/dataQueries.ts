...
export const playerStatsQueries = {
  search: (params: PlayerStatsSearchParams & { page?: number; pageSize?: number }) => createPaginatedQuery(
...
export const teamStatsQueries = {
  search: (params: TeamStatsSearchParams & { page?: number; pageSize?: number }) => createPaginatedQuery(
    `teamStats:search:${JSON.stringify(params)}`,
    () => internalApi.getTeamStats(params),
    params.pageSize || 18
  ),

  byTeam: (team: string, year: string) => createQuery(
  ...
  byConference: (conference: string, year: string) => createQuery(
...
export const gameQueries = {
  byTeams: (teams: string[], year: string, week: string) => createQuery(
  ...
  search: (params: GameSearchParams & { page?: number; pageSize?: number }) => createPaginatedQuery(
    `games:search:${JSON.stringify(params)}`,
    () => internalApi.getGames(params),
    params.pageSize || 16
  ),

  byId: (gameId: string) => createQuery(
    `games:byId:${gameId}`,
    () => internalApi.getGames({ id: gameId }),
    {
...
export const matchupQueries = {
  headToHead: (team1: string, team2: string, minYear?: string, maxYear?: string) => createQuery(
  ...
  recent: (team: string, count: number = 5) => createQuery(
...
export const teamQueries = {
  byDivision: (division: string = 'fbs') => createQuery(
  ...
  search: (query: string) => createQuery(
    `teams:search:${query}`,
    async () => {
